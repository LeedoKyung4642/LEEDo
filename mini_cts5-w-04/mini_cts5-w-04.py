...
#-----------  0:노란색, 1:빨강색, 3:파란색, 4: 000, 5:보라색, 6: 퍼렁색
color_num = [   0,  1,  2,  3,  4,  5,  6]
    
h_max =     [ 252, 65,196,111,110, 111, 51]
h_min =     [ 150,  0,158, 59, 74,  81, 32]
    
s_max =     [ 194,200,223,110,255, 146, 122]
s_min =     [ 113,140,150, 51,133, 127, 113]
    
v_max =     [  255,151,239,156,255, 141, 163]
v_min =     [   89,95,104, 61,104, 126, 122]
...
if __name__ == '__main__':
    ...
    cv2.createTrackbar('Color_num', Top_name,color_num[now_color], 6, Color_num_change) # 임의 컬러 보라색을 위해서 4->5로 변경하여 num 값 추가
    ...
    if serial_use != 0:
       ...
       #serial_port.flush() # serial cls
        thread = threading.Thread(target = readThread, args=(serial_port,))
        thread.start()
    #---------------------------
    ...
    while True:
        ...
        if len(cnts) > 0:
            ...
            #cv2.circle(frame, (int(X), int(Y)), int(radius), (0,0,255),2)  # 임의로 빨간원을 만듬 (190721)
            ...
            if Area > min_area[now_color]:
                ...
                print(" safe " + str(safe) + " gph " + str(gph) )
                print ("gph" )
                if gph == 1:
